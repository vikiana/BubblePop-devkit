...
var Physics = exports = Class(function () {
	...
	this.getHitBox = function () {
		...
		if (this.hitbox.hasOwnProperty ("radius")) {
			this._computedHitBox.radius = (this.hitbox ? this.hitbox.radius : this.style.height);
		}
	...
	this.getLineToCircleCollisions = function (group, line) {
		var collisions = [];
		if (!this._collisionsEnabled) {
			return collisions;
		}
		...
		if (uidmap) {
			for (var uid in uidmap) if (uid != this.uid) {
				var view2 = uidmap[uid];
				var hb = view2.getHitBox();
				if (view2.hasOwnProperty('layoutId'))
				{
					if (view2.layoutId == 52)
					{
						logger.LOG&&console.log("CHECK");
					}
				}
				var intersection = intersect.circleAndLine(hb, line);
				if (intersection) {
					collisions.push({
						intersection: intersection,
						view: view2
